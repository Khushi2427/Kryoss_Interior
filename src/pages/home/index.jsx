...
import { ArrowRight, Star, PlayCircle, Calculator, ChevronRight, ChevronLeft, Quote, UserCheck, ShieldCheck, ClipboardCheck, CreditCard, Clock, Trophy } from 'lucide-react';
...
const Home = () => {
    ...
    const prevReview = () => {
        setReviewIndex(prev => (prev === 0 ? homeReviews.length - 1 : prev - 1));
    };
            ...
            <section className="section" style={{ padding: '3rem 0' }}>
                <div className="container">
                    ...
                    <div style={{ position: 'relative', maxWidth: '800px', margin: '0 auto' }}>
                        <div
                            style={{ position: 'absolute', top: '50%', left: '-1rem', transform: 'translateY(-50%)', zIndex: 10, cursor: 'pointer', background: 'var(--kr-gold)', borderRadius: '50%', padding: '0.8rem', display: 'flex', boxShadow: '0 4px 12px rgba(0,0,0,0.3)' }}
                            onClick={prevReview}
                        >
                            <ChevronLeft size={24} color="#000" />
                        </div>
                        <div
                            style={{ position: 'absolute', top: '50%', right: '-1rem', transform: 'translateY(-50%)', zIndex: 10, cursor: 'pointer', background: 'var(--kr-gold)', borderRadius: '50%', padding: '0.8rem', display: 'flex', boxShadow: '0 4px 12px rgba(0,0,0,0.3)' }}
                            onClick={nextReview}
                        >
                            <ChevronRight size={24} color="#000" />
                        </div>

                        <div style={{ overflow: 'hidden', padding: '1rem 0', borderRadius: '16px' }}>
                            <div style={{ display: 'flex', transition: 'transform 0.6s cubic-bezier(0.25, 1, 0.5, 1)', transform: `translateX(-${reviewIndex * 100}%)` }}>
                                {homeReviews.map((review, i) => (
                                    <div key={i} style={{ minWidth: '100%', padding: '0 2rem', boxSizing: 'border-box' }}>
                                        <div className="blog-card" style={{ padding: '3.5rem 3rem', background: 'var(--kr-dark)', border: '1px solid rgba(255,255,255,0.1)', boxShadow: '0 10px 30px rgba(0,0,0,0.15)', position: 'relative', textAlign: 'center', height: '100%' }}>
                                            <div style={{ position: 'absolute', top: '1.5rem', left: '1.5rem', color: 'var(--kr-gold)', opacity: 0.4 }}>
                                                <Quote size={60} />
                                            </div>
                                            <div style={{ display: 'flex', justifyContent: 'center', color: 'var(--kr-gold)', marginBottom: '1.5rem' }}>
                                                <Star fill="var(--kr-gold)" size={20} /><Star fill="var(--kr-gold)" size={20} /><Star fill="var(--kr-gold)" size={20} /><Star fill="var(--kr-gold)" size={20} /><Star fill="var(--kr-gold)" size={20} />
                                            </div>
                                            <p style={{ fontStyle: 'italic', fontSize: '1.15rem', lineHeight: '1.8', color: '#eee', marginBottom: '2.5rem', minHeight: '110px', fontWeight: 300, position: 'relative', zIndex: 1 }}>
                                                <span style={{ color: 'var(--kr-gold)', fontSize: '1.4rem' }}>"</span>{review.text}<span style={{ color: 'var(--kr-gold)', fontSize: '1.4rem' }}>"</span>
                                            </p>
                                            <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '1.5rem', marginTop: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '1rem' }}>
                                                <div style={{ width: '50px', height: '50px', background: '#333', borderRadius: '50%', overflow: 'hidden', border: '2px solid rgba(255,255,255,0.1)' }}>
                                                    <img src={`https://ui-avatars.com/api/?name=${review.author.replace(/ /g, '+')}&background=c9a96e&color=000`} alt={review.author} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                                </div>
                                                <div style={{ textAlign: 'left' }}>
                                                    <div style={{ color: '#fff', fontSize: '1.2rem', fontFamily: 'Cormorant Garamond, serif', fontWeight: 600, letterSpacing: '0.04em', marginBottom: '0.1rem' }}>{review.author}</div>
                                                    <div style={{ fontSize: '0.8rem', color: 'var(--kr-red)', textTransform: 'uppercase', letterSpacing: '0.15em', fontWeight: 600 }}>{review.location}</div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'center', gap: '0.6rem', marginTop: '2rem' }}>
                            {homeReviews.map((_, i) => (
                                <div
                                    ...
                                    style={{
                                        width: i === reviewIndex ? '32px' : '8px',
                                        height: '8px',
                                        borderRadius: '4px',
                                        background: i === reviewIndex ? 'var(--kr-gold)' : 'rgba(255,255,255,0.2)',
                                        cursor: 'pointer',
